    ...
    const Chat = ({ currentUserName }) => {
        ...
        const handleSubmit = (e) => {
            ...
            const msgObj = {
                content: input,
                receiverId: selectedTargetId, // this is groupId or userId
        ...
        useEffect(() => {
            if (!hasSetDefault.current && groups.length > 0 && !selectedTarget) {
        ...
        const handleGroupScroll = (e) => {
            ...
            if (bottom && hasMoreGroups) loadMoreGroups();
        };
                ...
                <div className="flex flex-1">
                        ...
                        <div className="overflow-auto max-h-30" onScroll={handleGroupScroll} ref={groupListRef}>
                            ...
                            {groups.map((group) => (
                                <button
                                    ...
                                    onClick={() => {
                                        setSelectedTargetId(group.id);       // Use groupId for internal logic
                                        setSelectedTarget(group.name);  // For UI display
                                        setIsChannel(true);
                                    }}
                                >
                                    #{group.name}
                    ...
                    <main className="flex-1 flex flex-col justify-between p-4 bg-gray-50">
                        <div className="flex-1 overflow-y-auto mb-4"
                            ...
                            console.log("top", top);
                            if (top) loadOlderMessages(selectedTargetId,isChannel);
                        }}>
                            <AutoSizer>
                                {({ height, width }) => (
                                    <List
